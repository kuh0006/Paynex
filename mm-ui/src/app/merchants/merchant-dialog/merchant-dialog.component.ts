...
export class MerchantDialogComponent implements OnInit {
  ...
  initializeForm(): void {
    this.merchantForm = this.fb.group({
      ...
      name: [this.data?.merchant?.name || '', [Validators.required, Validators.minLength(3), Validators.maxLength(100)]],
      email: [this.data?.merchant?.email || '', [Validators.required, Validators.email]],
